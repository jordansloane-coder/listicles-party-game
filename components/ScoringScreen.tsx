...
interface Props {
  ...
  onContinue: () => void;
  onSkipBonus: () => void;
  onSetItemCounts: (playerId: string, itemIndex: number, counts: boolean) => void;
  onSubmitPlayerScore: (playerId: string) => void;
}
...
export default function ScoringScreen({
  ...
  const reviewingPlayer = players.find((p) => p.id === reviewingId) ?? null;
  const reviewingResult = reviewingId ? results.find((r) => r.playerId === reviewingId) : null;
      ...
      <div className="flex-1 flex flex-col px-5 py-8 gap-5 max-w-md mx-auto w-full">
        ...
        <div className="text-center animate-pop-in">
          ...
          <p className="text-sm opacity-60">
            {category} · bonus letter {bonusLetter}
          </p>
            ...
            <div key={i} className="rounded-2xl bg-card shadow p-3">
              ...
              {item.status === 'blank' ? (
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button
                    type="button"
                    onClick={() => onSetItemCounts(reviewingPlayer.id, i, true)}
                  ...
                  <button
                    type="button"
                    onClick={() => onSetItemCounts(reviewingPlayer.id, i, false)}
                    className={`rounded-xl py-2.5 font-bold text-sm active:scale-95 transition-transform ${
        ...
        <button
          onClick={() => {
            onSubmitPlayerScore(reviewingPlayer.id);
            setReviewingId(null);
          }}
  ...
  return (
    <div className="flex-1 flex flex-col px-5 py-8 gap-5 max-w-md mx-auto w-full">
      <div className="text-center animate-pop-in">
        ...
        <p className="text-sm opacity-60">Bonus letter: {bonusLetter} · tap a player to review and score their answers</p>
      </div>
        ...
        {players.map((player) => {
          ...
          return (
            <button
              key={player.id}
              type="button"
              onClick={() => setReviewingId(player.id)}
            ...
            >
              ...
              {result?.submitted ? (
                ...
                <span className="rounded-full bg-foreground/10 opacity-50 font-bold px-3 py-1 text-sm">Not reviewed →</span>
              )}
            </button>
          );
        })}
